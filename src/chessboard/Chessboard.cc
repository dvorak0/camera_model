                                               ...
                                               int flags)
{
    ...
    for (int k = 0; k < 1; ++k)
    {
        for (int dilations = minDilations; dilations <= maxDilations; ++dilations)
        {
            ...
            for (int group_idx = 0; !found; ++group_idx)
            {
                ...
                //printf("%lu, %s\n", quadGroup.size(), found ? "true" : "false");
...
{
    cv::Mat tmp_color_img = color_img.clone();
    ChessboardQuadPtr q;
    ...
    while (!stack.empty())
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (neighbor.get() && neighbor->count > 0 && neighbor->group_idx < 0)
            {
                ...
                for (int j = 0; j < 4; j++)
                {
                    sx += neighbor->corners[j]->pt.x;
                    sy += neighbor->corners[j]->pt.y;
                }
                //cv::circle(tmp_color_img, cv::Point2f{sx / 4, sy / 4}, 1, cv::Scalar(255, 0, 0), 2);
            }
        }
    }
    cv::imshow("connected", tmp_color_img);
}
                                ...
                                cv::Size patternSize)
{
    ...
    //puts("Write the corners in increasing order to the output file");
    for (int i = min_row; i <= max_row; ++i)
    {
    ...
    //puts("check that no corners lie at image boundary");
    float border = 5.0f;
