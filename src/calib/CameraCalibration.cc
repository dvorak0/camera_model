                                        ...
                                        std::vector<cv::Mat> &rvecs, std::vector<cv::Mat> &tvecs) const
{
    ...
    std::cout << camera->parametersToString() << std::endl;
                                 ...
                                 std::vector<cv::Mat> &rvecs, std::vector<cv::Mat> &tvecs) const
{
    ...
    for (size_t i = 0; i < rvecs.size(); ++i)
    {
        ...
        printf("%f %f %f %f %f %f %f\n",
               transformVec.at(i).rotation().w(),
               transformVec.at(i).rotation().x(),
               transformVec.at(i).rotation().y(),
               transformVec.at(i).rotation().z(),
               transformVec.at(i).translation().x(),
               transformVec.at(i).translation().y(),
               transformVec.at(i).translation().z());
        for (size_t j = 0; j < m_scenePoints.at(i).size(); j++)
        {
            Eigen::Vector3d P{m_scenePoints.at(i).at(j).x,
                              m_scenePoints.at(i).at(j).y,
                              m_scenePoints.at(i).at(j).z};
            Eigen::Vector2d p{m_imagePoints.at(i).at(j).x,
                              m_imagePoints.at(i).at(j).y};
            Eigen::Vector2d pp;
            camera->spaceToPlane(transformVec.at(i).rotation() * P + transformVec.at(i).translation(), pp);
            //printf("%lu %lu %f\n", i, j, (pp - p).norm());
        }
    }
}
